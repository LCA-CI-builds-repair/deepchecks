...
def default_fill_na_series(col: pd.Series, is_cat_column: t.Optional[bool] = None) -> t.Optional[pd.Series]:
    """Fill NaN values based on column type if possible, otherwise return None.

    Parameters
    ----------
    col : pd.Series
        The Series to fill NaN values for.
    is_cat_column : t.Optional[bool]
        Whether the column is categorical or not.

    Returns
    -------
    t.Optional[pd.Series]
        The Series with NaN values filled, or None if filling was not possible.
    """
