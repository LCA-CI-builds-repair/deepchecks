...
class ImagePropertyOutliers(AbstractPropertyOutliers):
    """Find outlier images with respect to the given properties.

    The check computes several image properties and then calculates the number of outliers for each property.
    The check uses the Interquartile Range (IQR) to detect outliers for single-dimensional properties.
    ...
    image_properties : List[Dict[str, Any]], default: None
        ...
        representing attributes of the method. 'output_type' must be one of:

        - 'numerical' - for continuous ordinal outputs.
        - 'categorical' - for discrete, non-ordinal outputs. These can still be numbers,
          but these numbers do not have inherent value.

        For more information on image/label properties, refer to the vision__properties_guide.
    ...
    iqr_percentiles: Tuple[int, int], default: (25, 75)
        Two percentiles that define the IQR range.
    iqr_scale: float, default: 1.5
        The scale to multiply the IQR range for outlier detection.
    ...
    n_samples: Optional[int], default: 10000
        Number of samples to consider for outlier detection.
